...
public sealed class EncounterCombatAutomaticTurnTests
{
    [Fact]
    public void AdvanceToDecision_DyingParticipant_ResolvesDeathSaveAndAdvances()
    {
        ApplicationSessionState source =
            EncounterCombatTestData.CreateDyingActiveSession(
                ...
                successes: 1,
                failures: 0);
        string actorId = EncounterCombatTestData.GetEncounter(source)
            .ActiveCombatantId;

        EncounterCombatResolutionResult result =
            EncounterCombatRules.AdvanceToDecision(source);

        EncounterCombatStepResult deathSave = result.AutomaticSteps[0];
        Assert.Equal(
            CombatStepKind.DeathSavingThrow,
            deathSave.Kind);
        Assert.Equal(actorId, deathSave.ActorCombatantId);
        EncounterCombatDieRoll die = Assert.Single(deathSave.Dice);
    ...
    public void AdvanceToDecision_NaturalOneDeathSave_AppliesTwoFailuresAndAdvances()
    {
        ...
        EncounterParticipantState participant =
            EncounterCombatTestData.GetParticipant(result.State, actorId);
    ...
    public void AdvanceToDecision_ThirdFailedDeathSave_KillsParticipantAndAdvances()
    {
        ...
        Assert.Equal(
            ...
            EncounterCombatTestData.GetEncounter(result.State).LifecycleState);
        Assert.Equal(
            CombatDecisionState.PlayerDecisionRequired,
            result.ResultingDecision.State);
        Assert.Equal(1, result.AutomaticSteps.Sum(step => step.Dice.Count));
    ...
    public void AdvanceToDecision_FinalDeathSaveFailure_CompletesImmediatelyWithoutLaterWork()
    {
        ...
        DeathSavingThrowResult mechanicalResult =
            coreResult.CombatantDeathSavingThrow
                .HealthDeathSavingThrow
                .DeathSavingThrow;
        EncounterState completed = EncounterCombatTestData.GetEncounter(
            result.State);
    ...
    public void AdvanceToDecision_NaturalTwentyDeathSave_ReturnsSameParticipantDecision()
    {
        ApplicationSessionState source =
            EncounterCombatTestData.CreateDyingActiveSession(
                ...
                successes: 1,
                failures: 0);
        EncounterState before = EncounterCombatTestData.GetEncounter(source);
        ...
        EncounterCombatStepResult step = result.AutomaticSteps[0];
    ...
    public void AdvanceToDecision_StableParticipant_AutomaticallyAdvancesWithoutDice()
    {
        ...
        source = EncounterCombatTestData.ReplaceParticipant(source, actor);
    ...
    public void AdvanceToDecision_MeleeRaider_AttacksNearestConsciousPartyTarget()
    {
        ...
        EncounterCombatStepResult attack = Assert.Single(
            result.AutomaticSteps,
    ...
    public void AdvanceToDecision_NearestUnattackableTarget_AttacksFartherLegalTarget()
    {
        ...
        source = EncounterCombatTestData.ReplaceEncounter(source, encounter);
        encounter = EncounterCombatTestData.GetEncounter(source);

        EncounterCombatAttackAvailability nearestPrerequisites =
        ...
        EncounterCombatStepResult movement = Assert.Single(
            result.AutomaticSteps,
    ...
    public void AdvanceToDecision_NoAttackableTarget_MovesForBestProgressWithoutAttacking()
    {
        ...
        Assert.DoesNotContain(
            ...
            step => step.Kind == CombatStepKind.WeaponAttack
                && step.ActorCombatantId
                    == "combatant.watchtower-raider.melee");
        EncounterCombatStepResult turn = Assert.Single(
            result.AutomaticSteps,
            step => step.Kind == CombatStepKind.TurnAdvanced
                && step.ActorCombatantId
                    == "combatant.watchtower-raider.melee"
    ...
    public void AdvanceToDecision_RangedRaiderWithIllegalAttack_DoesNotReposition()
    {
        ...
        EncounterCombatStepResult first = result.AutomaticSteps[0];
...
internal static class WatchtowerCombatStepListExtensions
{
    internal static int FindIndex(
        this IReadOnlyList<EncounterCombatStepResult> steps,
        Func<EncounterCombatStepResult, bool> predicate)
    {
