...
- [~] Phase 6 — Scenario content boundary (Watchtower → data). **Steps 1–5 done** (PRs #88–#99); step 6 (Console renders off the definition) and step 7 (prove it with a second scenario) remain.
  ...
  - **Known content finding:** nothing produces `SuccessReported` or `ScenarioCompleted`; both are referenced only by the save mapper. Reachability validation reports them as warnings and a test pins the finding. Open question whether Watchtower lacks a success path or the enum carries dead values.
